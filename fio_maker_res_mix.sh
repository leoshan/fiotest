...
do
	...
	printf "%-15s %-15s %-15s %-8s %-8s %-8s %-s" $iops $bwdata $delaydata $clat_50 $clat_90 $clat_99 $clat_9995 >> fio_res_output_mix.txt	
	printf "%-50s" $i >> fio_res_output_mix.txt

	echo  >> fio_res_output_mix.txt
done

cat fio_res_output_mix.txt
